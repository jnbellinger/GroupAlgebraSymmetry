...
See the section Conjugacy Classes below.  This is a special case of a more general
old theorem.

These continuous isomorphic transformations of dihedral groups $D_p$, $p>=3$ have a maximum of $3(p-1)/2$ 
independent parameters if $p$ 
...
\section{Conjugacy classes for $D_n$}
It is known that for $D_n$ if $n$ is odd, one has the identity, $(n-1)/2$
conjugacy classes of size 2, and all the reflections.  If $n$ is even one
has 2 conjugacy classes of size 1, and $n/2 - 1$ of size 2, and two
conjugacy classes for the reflections.  \footnote{Dihedral Groups,
Keith Conrad, https://kconrad.math.uconn.edu/blurbs/grouptheory/dihedral.pdf}
\par In either case the number of classes of order 2, and thus the number of
irreducible representations of size 4, is the same as the
limit I find above divided by 3.
\par In addition, $A4$ has 4 conjugacy classes, meaning three of size 1 and
one of size 3, and thus an irreducible representation of size 9.  $8=9-1$ is
evocative--there may be a simple way to predict the number of degrees of 
freedom the isomorphism of the group algebra onto itself may have.  
\footnote{Theory of Group Representations, page 97, M.A. Naimark and A.I. Stern}
"The group algebra of a finite group is symmetrically isomorphic to the direct sum
